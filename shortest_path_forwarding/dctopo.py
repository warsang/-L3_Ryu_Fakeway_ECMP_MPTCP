        ...
        for p in pods:
            ...
            for a in agg_sws:
                ...
                for c in core_sws:
                    ...
                    self.addLink(core_id, agg_id, bw = 1000)
        for h in host_list:
            edge = str(edge_Switch_Host_Table[h])[:-1]
            edge = edge[1:]
            if(edge == '_0_'):
                try:
                    self.addLink(h,edge_sw_list[edge_sw_list.index(edge_Switch_Host_Table[h]) + 1],bw = 1000)
                except:
                    pass
            elif(edge == '_1_'):
                try:
